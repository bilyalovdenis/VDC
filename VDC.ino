 ...
 */
  ...
  */
   ...
   * Задачи:
   * 1.Оформить класс для работы с датчиком дыхания:(Приступил)
   *    - создать алгорим измерения частоты дыхания с помощью изменения показателей давления в области груди животного;(алгоритм создан, не защищен от ошибок измерения, требуется проработка после испытания)
...
#define DT_PIN  A1                                          // указываем номер вывода, к которому подключен вывод DT (Датчик дыхания)
#define SCK_PIN A2                                          // указываем номер вывода, к которому подключен вывод SCK (Датчик дыхания)
...
class BreathVDC {                                          // Класс датчика дыхания(!!!РАЗОБРАТЬСЯ С КАЛИБРОВКОЙ, а после нее добавить функцию снятия показателей)
  ...
  public:
    ...
    void madeArrayBreath(int type_animal){                //возможно реализация в зависимости от типа животного(его размер) изменяется частота дыхания, а следователно частота измерений. Пока по стандарту
      for (int i = 0; i < 135; i++){
       arrayBreath[i]= _scale.get_units();
       delay (100);                                          //Установлен именно delay, так как код не должен выполняться дальше, пока идут измерения. Требуется калибровка задержки, так как неизвестно само время измерения.
      }                                        
    }
    uint8_t getFrequencyBreath(){
      frequency_breath = 0;
      for (int i = 1; i < 134; i++) {
        if (arrayBreath[i] > arrayBreath[i - 1] && arrayBreath[i] > arrayBreath[i + 1]) {
            frequency_breath += 1;
        }
    }
    return frequency_breath;
    }
  ...
  private:
    ...
    int arrayBreath [135];                                 // массив, хранящий показатели дыхания. Рассчитан на 9 измерений в секунду, в течении 15 секунд    
    int frequency_breath;                                   // переменная для хранения частоты дыхания
